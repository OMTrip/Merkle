...
export const getERC20TokenBalances = (address, chainId) => {
  return new Promise((resolve, reject) => {
    ...
    axios.get(`${MORALIS_BASE_API}${address}/erc20`, options).then(async (result) => {
      let tokens = await Promise.all(result.data.map(async (token) => {
        try {
...
export const getNFTs = (address, chainId) => {
  return new Promise((resolve, reject) => {
    const options = {
      params: {
        chain: chainId,
    ...
    axios.get(`${MORALIS_BASE_API}${address}/nft`, options).then((result) => {
      const nfts = result.data.result.map((nft) => {
        nft.chainId = chainId;
        return nft;
      });
...
export const coingecko_getChain = (chain) => {
  switch (chain) {
    case 'ethereum':
      return 'ethereum'
      break;
