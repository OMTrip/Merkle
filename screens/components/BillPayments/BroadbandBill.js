...
import { DTHOperator, broadband } from '../../../Utils/web3/helperFunction';
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.Card} onPress={()=>{navigation.navigate('DthPlan',{data:item, input:{name:"Customer Id", type:"DTH" }})}}>
        <Carditems
          ...
          source={{
            uri: broadband[`${item.operator_name}`]
              ? broadband[`${item.operator_name}`]
              : 'https://res.cloudinary.com/dpe8nipmq/image/upload/v1695623662/nute/broadband/Asset24_nlbyqd.png',
          }}
        />
      </TouchableOpacity>
  );
  ...
  useEffect(() => {
    ...
    if (data?.length) {
      const dthOp = data?.filter((ele, i) => {
        return ele.service_type === 'Broadband' && ele.biller_status == 'on';
      });
